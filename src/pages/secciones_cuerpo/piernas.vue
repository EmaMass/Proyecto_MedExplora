<template>
  <v-container fluid class="pa-0 fill-height">
    <v-card class="d-flex flex-column" style="width: 100%; height: 100%;">
      <v-card-title class="flex-grow-0" style="background-color: #283593 !important; color: white; transition: background-color 0.6s ease;">
        <v-icon start>mdi-human-male-height</v-icon>
...
const loadModel = () => {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      model.traverse((child) => {
        if (child.isMesh) {
          ...
          child.material.color.setHex(0x283593);
          child.material.emissive.setHex(0x1a1f5a);
        }
      });

      isLoadingModel.value = false;
      console.log('Modelo de piernas cargado exitosamente');
    },
    (progress) => {
    ...
    (error) => {
      console.error('Error al cargar el modelo de piernas:', error);
      isLoadingModel.value = false;
      errorMessage.value = 'No se pudo cargar el modelo. Verifica que legs_model.glb existe en src/assets/';
    }
  );
};
